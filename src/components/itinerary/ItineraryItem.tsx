...
export function ItineraryItem({ item, isDragging, dragHandleProps, onDelete }: Props) {
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-xl border p-3 flex gap-3 items-start transition-shadow ${
        isDragging ? "shadow-lg" : "border-gray-100"
      }`}
    >
      {/* Drag handle */}
      <div {...dragHandleProps} className="text-gray-300 text-lg mt-0.5 cursor-grab active:cursor-grabbing select-none">
        ⋮⋮
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className="text-sm font-semibold text-gray-900 leading-snug">{item.place_name}</p>
          {item.start_time && (
            <span className="text-xs text-gray-400 shrink-0 mt-0.5">{item.start_time.slice(0, 5)}</span>
          )}
        </div>
        {item.notes && (
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          {item.category === "transport" && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs px-2 py-0.5 rounded-full bg-orange-600 text-white font-medium"
            >
              RedBus
            </a>
          )}
      ...
      {/* Delete */}
      {onDelete && (
        <button onClick={onDelete} className="text-gray-300 hover:text-red-400 transition-colors text-sm shrink-0">
          ✕
        </button>
      )}
    </motion.div>
